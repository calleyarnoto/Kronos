...
def calculate_prediction_parameters(df, target_days=15):
    """
    根据目标预测天数计算合适的参数

    参数:
    df: 股票数据DataFrame
    target_days: 目标预测天数（自然日），默认15天
                 注：原默认值为20天，进一步缩短至15天
                 个人测试发现15天预测在A股市场上准确率更高，
                 超过20天误差会显著增大
    ...
    pred_len = min(pred_trading_days, len